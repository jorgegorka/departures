# TalentoHQ Style Guide

This document defines the CSS architecture, design tokens, and styling conventions used throughout TalentoHQ.

## Overview

TalentoHQ uses **pure, custom CSS** with no external frameworks (no Tailwind, Bootstrap, etc.). The design system is built on modern CSS features including CSS custom properties, OKLCH colors, CSS layers, and logical properties.
...
TalentoHQ uses the OKLCH color space for better perceptual uniformity across light and dark modes. Colors are defined as lightness, chroma, and hue values:
...
| `--color-surface` | Raised/content surface background |
| `--color-surface-muted` | Quiet surface background |
| `--color-surface-subtle` | Subtle grouped surface background |
| `--color-border` | Standard divider and card border |
| `--color-border-muted` | Low-contrast divider |
| `--color-border-medium` | Form and table divider border |
| `--color-border-strong` | Form-control border |
...
| `--color-brand`, `--color-brand-strong` | Primary product accents |
| `--color-info`, `--color-info-strong` | Informational and blue accent states |
| `--color-success`, `--color-success-strong` | Success states and positive indicators |
| `--color-warning` | Warning states and yellow indicators |
| `--color-danger`, `--color-danger-strong` | Error/destructive states |
| `--color-critical` | Critical rose-toned error states |
| `--color-accent`, `--color-accent-strong` | Secondary purple accents |

### Color Palette

The palette includes semantic OKLCH tokens for product colors and channel-based
named palette tokens for repeated swatches/status accents.
...
For repeated named swatches and status colors, use the channel tokens in
`base.css`:
...
box-shadow: 0 0 0 3px rgb(var(--palette-blue-200) / 0.6);
```

Palette tokens centralize legacy feature accents and user-selectable color
sets without creating Tailwind-style utility classes. Do not add new raw
`rgb(...)` color values in feature CSS; add a token first when a reusable
palette value is needed.
...
TalentoHQ uses CSS logical properties for RTL support. Use `block` (vertical) and `inline` (horizontal) instead of top/bottom/left/right.
...
Buttons use `.btn` plus explicit style and size modifiers. Shared button
styles live in `app/assets/stylesheets/buttons.css`; keep new variants there
instead of creating local utility-style escapes in feature CSS.

```html
<button class="btn btn--plain btn--medium">Cancel</button>
<button class="btn btn--primary btn--medium">New record</button>
<button class="btn btn--secondary btn--large">Save changes</button>
```
...
| Class | Purpose |
|-------|---------|
| `.btn--plain` | Neutral cancel, back, and lower-emphasis actions |
| `.btn--primary` | Primary indigo action, usually navigation or creation |
| `.btn--secondary` | Affirmative green submit action, commonly Save |
| `.btn--action` | Soft indigo secondary action next to stronger controls |
| `.btn--edit` | Edit action where a distinct teal channel is already used |
| `.btn--destroy` | Destructive action with visible danger styling |
| `.btn--soft-*` | Compact low-emphasis action chips for tables and empty states |
| `.btn--print` | Print/export button with neutral surface styling |

**States:**
- Hover: subtle lift, stronger border or shadow, and no layout shift
- Focus-visible: visible ring or shadow, never color alone
- Active: pressed shadow and no upward transform
- Disabled: reduced opacity, no pointer events, and no hover lift
- Contrast: action text must stay at or above 4.5:1 against every gradient stop
...
TalentoHQ uses a **CSS mask-based icon system** with individual SVG files. Icons inherit color from their parent via `currentColor`, making them easy to style contextually.
...
**Application:**
`talentohq`, `boost`, `boost-color`, `golden-ticket`

---

## Forms & Inputs

TalentoHQ uses CSS custom properties for flexible, themeable form controls. All inputs prevent iOS auto-zoom with `font-size: max(16px, 1em)`.
...
.borderless
.border-radius       /* Uses component-level radius, default 0.5em */
```
...
## Migration Completion Guard

The Tailwind-to-modern-CSS migration is complete. Historical progress is tracked in `docs/plans/2026-04-30-legacy-tailwind-removal-roadmap.md` and `docs/plans/2026-05-06-legacy-tailwind-next-batches.md`. Use the commands below as regression guards when changing views, components, helpers, JavaScript class emitters, or stylesheets.

### Guard Commands

```bash
# Files using common Tailwind-style utility classes should stay at zero.
rg -l 'class="[^"]*(\bflex\b|\bgrid\b|\btext-(sm|lg|xs|gray-)|\bp[xy]?-[0-9]|\bm[txy]?-[0-9]|\bw-[0-9]|\bh-[0-9]|\bbg-(white|gray-))' \
  app/views app/components

# Legacy Tailwind bundle should stay removed
test ! -e app/assets/stylesheets/legacy_tailwind.css

# No legacy layer or Tailwind custom-property residue in app code
rg '@layer legacy|Tailwind preflight|--tw-' app/assets/stylesheets app/views app/components app/helpers app/javascript app/lib

```

### Migration Baseline And Final State

| Metric                                             | Value  |
| -------------------------------------------------- | ------ |
| Baseline `legacy_tailwind.css` lines (2026-04-30)  | 11,737 |
| Baseline ERB files with utility hits (2026-04-30)  | 1,071  |
| Final `legacy_tailwind.css` state (2026-05-08)     | Deleted |
| Final view/component utility audit (2026-05-08)    | 0 files |

Hot zones at baseline: `app/views/admin/` (~618 files), `app/views/main/` (~231 files).

### Foundation files

The architecture sketched at the top of this document is real now:

- `app/assets/stylesheets/base.css` declares cascade layer order (`base, utilities`), the global reset/form-control baseline, and the design tokens (`--block-space`, `--text-*`, `--color-*`, font stacks). Component CSS and utility CSS pull from these.
- `app/assets/stylesheets/utilities.css` provides the curated layout/spacing/typography/state primitives listed above. Wrapped in `@layer utilities` so unlayered BEM rules win on collision.
- `app/assets/stylesheets/legacy_tailwind.css` has been removed. Do not recreate it or add Tailwind-style utility escape hatches.
